...
5. Playbooks go under `content/docs/playbooks/` with the `<Callout
   type="info" title="Playbook">` intro described above.
6. Run `npm run build` before committing.

### Theme

Fumadocs' built-in `ocean` preset (`fumadocs-ui/css/ocean.css`), imported in
`app/globals.css` alongside Tailwind CSS v4 and `fumadocs-ui/css/preset.css`.
